...
| ファイル | Story | 用途 |
|---------|-------|------|
| `notebooks_sprint4/story4-1/nb_01_eda_data_overview.py` | 4-1 | Gold 層テーブルの概要・件数・基本統計量の確認 |
| `notebooks_sprint4/story4-1/nb_02_eda_sales_analysis.py` | 4-1 | 時系列トレンド・分布・相関分析・特徴量決定 |
| `notebooks_sprint4/story4-2/nb_01_automl_data_prep.py` | 4-2 | 学習データセットの作成と保存 |
| `notebooks_sprint4/story4-2/nb_02_automl_run.py` | 4-2 | AutoML 実験の実行と結果確認 |
...
> **ML ランタイム推奨**: クラスター作成時に **Databricks Runtime ML** を選択すると、MLflow・scikit-learn・XGBoost 等がプリインストールされる。既存クラスターが Standard Runtime の場合でも AutoML は動作するが、ML Runtime のほうが追加ライブラリ不要で便利。

### Step 1-2: Gold 層テーブルの概要確認

以下のノートブックを Databricks ワークスペースにインポートして実行する。

`notebooks_sprint4/story4-1/nb_01_eda_data_overview.py`
...
`notebooks_sprint4/story4-1/nb_02_eda_sales_analysis.py`
...
`notebooks_sprint4/story4-2/nb_01_automl_data_prep.py`
...
`notebooks_sprint4/story4-2/nb_02_automl_run.py`
...
> **保存先**: `docs/01_management/sprint4/証跡/` 配下に保存する。
