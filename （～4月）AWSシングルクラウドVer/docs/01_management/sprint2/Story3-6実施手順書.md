...
## Phase 5 おまけ: メタデータ参照検証テスト — カナリアトークン法（Story 3-6）

### 背景と目的

Phase 5 のテストでは全テスト項目に対し正しい結果が得られた。しかし、これは **ビジネスメタデータを Genie が実際に参照したから正しく答えられた** とは言い切れない。Northwind データはカラム名・値が十分に自明であり、メタデータがなくても正解できた可能性がある。

本テストの目的は、**Genie がメタデータを確実に参照していることを立証する**ことである。

### アプローチ: カナリアトークン法

「カナリアトークン」とは、炭鉱のカナリア（異常検知の比喩）に由来する概念で、**ここでは Unity Catalog のメタデータに意図的に埋め込む "仕掛け" の情報**を指す。

この特殊な情報は**データ自体には存在せず、メタデータ（コメント・同義語・テキスト指示）にのみ記載される**。Genie がこの情報を回答に反映すれば、メタデータを参照した証拠となる。反映しなければ、メタデータが無視されている可能性がある。

### Step おまけ-1: カナリアトークンの設計

以下の 3 種類のカナリアトークンを用意する。各トークンは **現実のデータには絶対に存在しない架空の情報** である。

| # | 種類 | 埋め込み先 | カナリアトークン内容 | テスト質問 |
|---|------|-----------|---------------------|-----------|
| C1 | カラムコメント | `silver.products` の `discontinued` カラムのコメント | 元のコメントに追記: `「社内では販売終了商品のことを "サンセット商品" と呼ぶ」` | 「サンセット商品の一覧を見せて」 |
| C2 | テキスト指示 | Genie Space のテキスト指示に追記 | `「"プレミアム顧客" とは、累計注文件数が10件以上の顧客のことを指す」` | 「プレミアム顧客の一覧を教えて」 |
| C3 | SQL 式（Measure） | SQL Expressions に新規追加 | 名前: `配送コスト比率` / コード: `SUM(orders.freight) / SUM(order_details.unit_price * order_details.quantity * (1 - order_details.discount)) * 100` / 同義語: `物流コスト率, 運賃比率` | 「配送コスト比率を国別に教えて」 |

> **設計のポイント**: 「サンセット商品」「プレミアム顧客」「配送コスト比率」はいずれも **Northwind のデータやカラム名には一切登場しない用語・概念** である。Genie がこれらを正しく解釈できたなら、メタデータを参照した以外に説明がつかない。

### Step おまけ-2: カナリアトークンの埋め込み

#### C1: カラムコメントへの埋め込み

1. **Configure** > **Data** > `products` テーブルをクリック
2. `discontinued` カラムの鉛筆アイコンをクリック
3. 既存の説明に以下を **追記** する:

```
社内では販売終了商品のことを「サンセット商品」と呼ぶ。
```
...
1. **Configure** > **Instructions** > **Text** タブ
2. テキスト指示の末尾に以下を追記:

```
- ビジネス用語:
  - 「プレミアム顧客」とは、累計注文件数が10件以上の顧客を指す。COUNT(DISTINCT orders.order_id) >= 10 で判定すること。
```
...
|------|-----|
| Name | 配送コスト比率 |
| Code | `SUM(orders.freight) / SUM(order_details.unit_price * order_details.quantity * (1 - order_details.discount)) * 100` |
| Synonyms | 物流コスト率, 運賃比率 |
| Instructions | 売上金額に対する運賃の比率（%）。国別・期間別の物流効率を評価する指標。 |

4. **Save**

### Step おまけ-3: テストの実行

**New chat** を開き、以下の質問を **1 つずつ** 実行する。

| # | テスト質問 | 期待する結果 | 判定基準 |
|---|-----------|-------------|---------|
| C1 | 「サンセット商品の一覧を見せて」 | `discontinued = 1` の商品リストが返る | `WHERE discontinued = 1` 相当の SQL が生成されれば **PASS** |
| C2 | 「プレミアム顧客の一覧を教えて」 | 注文件数 10 件以上の顧客リストが返る | `HAVING COUNT(DISTINCT order_id) >= 10` 相当の条件があれば **PASS** |
| C3 | 「配送コスト比率を国別に教えて」 | 国別の運賃/売上比率（%）が返る | 登録した Measure の計算式が使われていれば **PASS** |

各回答の **Show code** を必ず展開し、生成された SQL を確認すること。

### Step おまけ-4: 結果の評価

| 結果パターン | 解釈 |
|-------------|------|
| **3/3 PASS** | メタデータ（コメント・テキスト指示・SQL 式）が確実に参照されている。Phase 5 の正答もメタデータに裏付けられている可能性が高い |
| **2/3 PASS** | 部分的に参照されている。FAIL した種類のメタデータは Genie が十分に活用できていない可能性がある。記述を改善して再テストを推奨 |
| **1/3 以下** | メタデータの参照が不十分。Genie Space の設定や記述方法に問題がある可能性がある。Phase 3 に戻って全体を見直すこと |

> **補足**: C3（SQL 式）は Genie の仕組み上、最も確実に参照される。C1（カラムコメント）は RAG の検索精度に依存するため、失敗しても記述の改善で改善される余地がある。C2（テキスト指示）はグローバル指示としての重み付けを検証している。

### Step おまけ-5: カナリアトークンの除去（テスト後の原状復帰）

テスト完了後、以下を**必ず実施**する。カナリアトークンは検証目的のみの情報であり、本番運用に残してはならない。

#### C1 の除去

1. `products` > `discontinued` のコメントから「サンセット商品」の追記部分を削除
2. **Save**

#### C2 の除去

1. テキスト指示から「プレミアム顧客」の記述を削除
2. **Save**

#### C3 の除去（任意）

`配送コスト比率` の SQL 式は、**ビジネス上有用であれば残してもよい**。不要であれば削除する。

1. **SQL Expressions** タブで `配送コスト比率` を選択し **Delete**

> **重要**: 除去を忘れると、Phase 6 で共有したユーザーに「サンセット商品」等の社内用語として誤認識される恐れがある。テスト証跡を記録したら速やかに除去すること。

### 証跡の記録

テスト結果を証跡として残す場合、以下をスクリーンショットに含めること：

1. 各カナリアトークンの埋め込み画面（Before / After）
2. 各テスト質問の回答画面（生成された SQL の **Show code** を展開した状態）
3. カナリアトークン除去後の画面
