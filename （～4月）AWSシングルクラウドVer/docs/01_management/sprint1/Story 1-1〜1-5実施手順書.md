...
現在の `02_etl_bronze_ingest.py` は `mode("overwrite")` でテーブル全体を上書きすることで冪等性を確保している。以前の `replaceWhere` 方式は同日再実行の冪等性は保証するが、異なる日に実行すると過去日のデータが残り主キーが重複するため廃止した。
...
1. Databricks UI 左メニューから **jobs※ジョブとパイプライン** を開く
2. **Create job** をクリック
...
1. **Add schedule（スケジュールとトリガー）** をクリック
2. 以下の通り設定する

| 設定項目 | 値 |
|---------|-----|
| Trigger type | Scheduled |
| Schedule | 毎日 21:00（JST） |
| Quartz cron 式 | `0 0 2 * * ?` |
| Timezone | Asia/Tokyo |
...
1. タスクをクリック → **コンピュート** 欄の編集ボタンをクリック
2. **Create new job cluster（新しいジョブクラスタを追加）** を選択
3. 以下のパラメータを設定する

| 設定項目 | 値 |
|---------|-----|
|コンピュート名|northwind_job_cluster|
|ポリシー|Job Compute|
| Databricks Runtime | `デフォルト` |
...
1. Job 編集画面の **Notifications（ジョブ通知）** タブをクリック
2. **Add notification（通知の追加）** をクリック
3. 以下の通り設定する
...
| 項目 | 内容 |
|------|------|
| overwrite の前提 | `mode("overwrite")` はテーブル全体を上書きするため、ソースデータの全量取得が前提。差分取込には適さない |
