...
| **AWS のサービス → EventBridge** | ⚠️ **Classic EventBridge Rules 用**。`AmazonEventBridgeApiDestinationsServiceRolePolicy` が付与される。EventBridge **Scheduler** には使わない |
| カスタム信頼ポリシー | EventBridge Scheduler ロールを手動作成する場合（後述）。通常は Scheduler コンソール自動作成を推奨 |

> **EventBridge Scheduler のロールは IAM Console から直接作らない**
>
> IAM Console の「AWS のサービス」ユースケース一覧に「EventBridge Scheduler」はなく、「EventBridge」を選ぶと Classic Rules 用のロールが作成されてしまいます（信頼プリンシパルが `events.amazonaws.com`）。
>
> Scheduler 用ロール（信頼プリンシパル: `scheduler.amazonaws.com`）を最も確実に作成する方法は、**EventBridge Scheduler コンソールのスケジュール作成画面で「このスケジュール用に新しいロールを作成」を選択する**ことです。AWS がターゲット Lambda に最小権限のポリシーと正しい信頼ポリシーを自動設定します。
...
### EventBridge Scheduler ロール（推奨: コンソール自動作成）

```
EventBridge → スケジュール → スケジュールを作成
→ ターゲット: Lambda → 関数を選択
→ 設定ページ:「このスケジュール用に新しいロールを作成」を選択
→ ロール名を入力（例: northwind-eventbridge-nat-role）
→ AWS が自動で以下を設定:
   - 信頼ポリシー: scheduler.amazonaws.com
   - 許可ポリシー: lambda:InvokeFunction（対象 Lambda のみ）
```

> IAM Console でどうしても手動作成する場合:
> 「カスタム信頼ポリシー」を選択し、信頼ポリシー JSON に `scheduler.amazonaws.com` を記載する。
> その後、ポリシーを先行作成してアタッチする（Lambda 実行ロールと同じ手順）。
...
| 2026-03-27 | EventBridge Scheduler ロールの誤情報を修正（「AWS のサービス → EventBridge」は Classic Rules 用と明記、Scheduler コンソール自動作成を推奨） |
